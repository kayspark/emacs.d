...
| Startup | ~0.5s batch, ~0.57s daemon (M5 Max ARM64). Full: 372 features, Denote: 306 features |
...
Fonts: Iosevka SS14 (base) + Paperlogy (Korean hangul) + NanumGothicCoding (fixed-pitch Korean).
Modeline: doom-modeline (deferred to ~after-init-hook~ for faster startup). Git gutters: diff-hl.
Relative line numbers enabled globally.
Daemon warm-up: pre-initializes hidden NS frame at 3s to make first ~emacsclient -c~ instant.

*Packages:* nepes-themes (vc), doom-modeline, diff-hl, rainbow-mode
...
Tree-sitter: treesit-auto (grammar auto-install, deferred to ~after-init-hook~). Direnv: envrc.
...
Babel: emacs-lisp, Python, Shell, SQL, LaTeX, R — all lazy-loaded on first block execution.
...
- Full daemon loads ~70 more features than denote daemon (372 vs 306). Full starts in ~0.57s on M5 Max ARM64. Use =--daemon=denote= for isolation (no envrc, treesit-auto, magit hooks).
- See [[file:docs/2026-03-22-daemon-perf-tuning.org][Daemon Performance Tuning]] for profiling methodology and optimization decisions.
