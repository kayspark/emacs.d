...
| Startup | ~0.7s batch, ~2s GUI, ~33s daemon (full), <1s daemon (denote) |
...
#+begin_src bash
emacs --daemon              # Full daemon (~33s, all 47 packages)
emacs --daemon=denote       # Lightweight daemon (<1s, 12 packages — for Raycast)
emacsclient -c              # GUI frame (connects to default daemon)
emacsclient --socket-name=~/.config/emacs/server/denote -c  # Connect to denote daemon
#+end_src

Both daemons are managed by launchd (=RunAtLoad + KeepAlive=):

| Daemon  | LaunchAgent                     | Socket                          | Startup |
|---------+---------------------------------+---------------------------------+---------|
| default | =org.gnu.emacs.daemon=          | =~/.config/emacs/server/server= | ~33s    |
| denote  | =org.gnu.emacs.denote=          | =~/.config/emacs/server/denote= | <1s     |
...
- Full daemon startup ~33s: all packages eager-loaded. Use =--daemon=denote= (<1s) for note-only workflows.
